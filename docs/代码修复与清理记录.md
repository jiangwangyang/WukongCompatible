...
四个文件各做三处同款改动(ThrustHeavyAttack.java、PillarHeavyAttack.java、SmashHeavyAttack.java、GreatSageHeavyAttack.java):

| 改动 | 说明 |
|---|---|
| drawOnGui 开头补 RenderSystem.enableBlend() + defaultBlendFunc() + setShaderColor(1,1,1,1) | 显式建立混合状态, 不再依赖上游; 着色器颜色复位 |
| 新增 drawLightTexture 方法, 金/白/红光晕绘制点全部改调它 | 光晕以星点为中心按 20x20 缩小绘制(原为 48x48 整图), 光斑直径约 19px 收敛到约 8px; 大小由方法内 size = 20 一处常量控制 |
| drawTexture 与 progress 环的 blit 参数 2,2,2,2 修正为 256,256,256,256 | 与贴图实际尺寸一致, 消除隐式依赖 |

同时为四个文件补充 import com.mojang.blaze3d.systems.RenderSystem。GreatSageHeavyAttack 的 drawTexture/drawLightTexture 为 private static(与该文件原风格一致), 其余三个为实例方法。四个文件均已按项目约定用 google-java-format(AOSP 风格)格式化。
