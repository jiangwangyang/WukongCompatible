...
跳星消费块沿用立棍/戳棍原语义: `if (stack < 3) { setStackSynchronize(min(stack+2, 4)); 播 XULI_LEVEL[新星级-1]; 同步 LAST_STACK; } 清 flag`。3 星及以上格挡只清标记不加星。

## 3. 统一后的四式行为(完全一致)

1. **跳星**: 铜头铁臂成功格挡 → 星数 ≤2 时 +2(封顶 4), ≥3 时不变;
...
| 场景 | 修改前 | 修改后 |
|---|---|---|
| 劈棍/大圣铜头铁臂格挡成功 | 无跳星(SMASH 显式无加成, GREATSAGE 落空) | 星 ≤2 时 +2, 与立棍/戳棍一致 |
...
1. 四种棍式下铜头铁臂格挡成功 → 0→2 / 1→3 / 2→4 跳星, 3 星不变;
