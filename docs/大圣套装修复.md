...
2. **重写 tools/convert_dasheng_armor.py**,生成 4 个 `animmodels/armor/dasheng_{h,c,l,f}.json`:
...
| 转换脚本 v5 | tools/convert_dasheng_armor.py(Blender 坐标存储 + 无平移 + parts 预三角化 + 角表修正 + 官方混合曲线) | 历次问题: Z 轴未取负、错误平移、未预三角化、blf/brf 角表写反 | 生成几何/蒙皮/UV/拓扑都正确的 Epic Fight 网格 |
