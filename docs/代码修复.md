...
> 本文档记录本项目（黑神话:悟空主题 Minecraft 模组，MC 1.20.1 + Forge + Epic Fight 20.14.17）的全部功能性 bug 修复，共十个部分。
> 前四部分整合自四份历史文档（原文件已删除，内容完整保留）：
...
> 本文档原名《代码修复与清理记录》，现整理为《代码修复》；不改变运行行为的代码清理/重构/注释统一内容见姊妹篇 docs/代码重构.md。
...
| 第七部分 | 铜头铁臂跳星扩展到四种棍式 + 四蓄窗口(降星时间)统一 | 本次新增(2026-09-09) |
| 第八部分 | 棍势衰减时机修复: 命中刷新四蓄窗口不再限于满 4 星 | 本次新增(2026-09-11) |
| 第九部分 | 戳棍寸退闪避修复(窗口前移+残留清除) + 进尺闪避联动 + 动画比较恒 false 修复 | 本次新增(2026-09-13) |
| 第十部分 | 斩棍式 0 星崩溃修复 + 切手技无棍势弱化版统一 + 江海翻命中解锁与耗全部棍势增伤 + 搅棍重击后入口 | 本次新增(2026-09-13) |
...
除上述 7 个问题外, 同一批移植修复中的"大圣套装碎片"问题因内容独立且篇幅较长, 已单独归档于 docs/大圣套装修复.md; "聚形散气瞬移/定字"问题见本文第二部分。
...
移植到 Epic Fight 20.14.17 时(提交 e8e2d22), 这份代码被改写成注入新版方法 renderWeaponInnateSkill, 但它的登记条目没有保留下来, 于是变成了死代码。本文第三部分记录过这一"搁置状态"。
...
- 死代码判定: src/main/resources/wukong.mixins.json(本仓库当前内容)与 git 历史(自引入本仓库的首个提交 e8e2d22 起即无此条目); 本文第三部分当时现状记录;
...
3. 本文第三部分中关于搁置状态的描述是对当时现状的历史记录, 保留原文未改, 以本部分为最新结论;
...
# 第七部分: 铜头铁臂跳星扩展到四种棍式 + 四蓄窗口(降星时间)统一（2026-09-09）

## 1. 背景与问题（四模式审查结论）

对劈棍/立棍/戳棍/大圣四种模式的四蓄(4星)机制做全链路审查, 结论是**互不统一且主线路径全部失效**, 死法分两类:

1. **铜头铁臂跳 2 星只有立棍/戳棍可用**: `ShenfaTongtoutiebiSkill` 成功格挡后仅置 `PILLAR_FASHU_STACK` / `THRUST_FASHU_STACK`; SMASH 分支显式注释"无铜头铁臂加成", GREATSAGE 无对应分支直接落空;
2. **"满 4 星命中 → CHARGED4_TIMER=300 → 15 秒后降回 3 星"四式全部从未生效**:
   - 劈棍/大圣: 代码链完整(置数→递减→降星), 但二者自身无到 4 星的路径(breakProgress 阈值 0.3/0.5/0.7 封顶 3 星, 又无铜头铁臂跳星), `isFull()` 恒假 → 置数点不可达, 整链事实死代码;
   - 立棍/戳棍: 4 星可达(铜头铁臂 min(stack+2,4)), 但类内无置数点 → timer 恒 0 → 降星分支永不触发, 4 星常驻; 副作用是 timer==0 且 ≥3 星的缓慢耗条分支持续生效;
   - 戳棍另用独立 key `Thrust_CHARGED4_TIMER`, 与其余三式的共用 key `CHARGED4_TIMER` 割裂;
3. **原版 mod(wukong-forge1.20.1-20.2.0.jar)反汇编对照**: 戳棍/立棍置数点原版就缺(死代码系继承而非重写回归); 大圣置数点为重写时新增(原版没有); `GREATSAGE_CHARGED4_TIMER` 为定义后零引用的死 key;
4. 立棍无法"蹭"劈棍的置数监听: 切式经 `SkillContainer.setSkill` 调旧技能 `onRemoved`(反编译 EpicFight-20.14.17.jar 确认), 监听器互斥。

## 2. 修改内容明细

| 文件 | 改动 |
|---|---|
| Config.java | 新增 `charged4_window_ticks`(DoubleValue, 默认 300, 注释"20 ticks = 1 秒"), 四式统一的降星时间配置项 |
| WukongSkillDataKeys.java | 新增 `SMASH_FASHU_STACK` / `GREATSAGE_FASHU_STACK`(BOOLEAN, owner 各自类); 删除死 key `Thrust_CHARGED4_TIMER` / `GREATSAGE_CHARGED4_TIMER` |
| ShenfaTongtoutiebiSkill.java | SMASH 分支改置 `SMASH_FASHU_STACK`, 新增 GREATSAGE 分支置 `GREATSAGE_FASHU_STACK` |
| SmashHeavyAttack.java | updateContainer 服务端分支最前加铜头铁臂跳星消费块(与立棍/戳棍同款: <3 星时 min(stack+2,4) + XULI_LEVEL 音效 + LAST_STACK 同步); 置数点改用 Config.CHARGED4_WINDOW_TICKS; 删除 MAX_CHARGED4_TICKS 常量 |
| PillarHeavyAttack.java | onInitiate 新增 `DEAL_DAMAGE_EVENT_DAMAGE` 监听(isFull → CHARGED4_TIMER = 配置值), onRemoved 对应移除 |
| ThrustHeavyAttack.java | 同上补置数监听; 递减/降星逻辑从 `Thrust_CHARGED4_TIMER` 切换到共用 key `CHARGED4_TIMER` |
| GreatSageHeavyAttack.java | ADD_BEANS 块后加同款跳星消费块; 置数点改用 Config; 删除私有 MAX_CHARGED4_TICKS 常量 |

跳星消费块沿用立棍/戳棍当时的门槛写法: `if (stack < 3) { setStackSynchronize(min(stack+2, 4)); 播 XULI_LEVEL[新星级-1]; 同步 LAST_STACK; } 清 flag`。该门槛经实测反馈后放宽为 `stack < 4`(3 星亦可上 4 星), 见本部分第 7 节。

## 3. 统一后的四式行为(完全一致)

1. **跳星**: 铜头铁臂成功格挡 → 星数 <4 时 +2 并封顶 4(0→2、1→3、2→4、3→4), 满星时只清标记;
2. **开窗**: 满 4 星(isFull)命中敌人 → `CHARGED4_TIMER = charged4_window_ticks`(默认 300 = 15 秒); 窗口内每次命中刷新(2026-09-11 更新: 刷新条件已放开为任意星级命中即刷新, 见第八部分);
3. **降星**: 窗口计时自然走完且仍满星 → 降回 3 星, 蓄力条回退一小格(max - CHARGING_SPEED/5);
4. **漏气**: timer==0 且 ≥3 星时每 tick 缓慢消耗蓄力条 CHARGING_SPEED/5(原有分支, 现在四式统一语义下生效);
5. **切式连续**: stack 与 CHARGED4_TIMER 均为共用 data key, 跨式切换时窗口计时连续(UpdateWeaponInnatePacket 保留 stack, data 值随容器保留)。

## 4. 修改前后对比

| 场景 | 修改前 | 修改后 |
|---|---|---|
| 劈棍/大圣铜头铁臂格挡成功 | 无跳星(SMASH 显式无加成, GREATSAGE 落空) | 星 <4 时 +2 封顶 4, 与立棍/戳棍一致 |
| 立棍/戳棍 4 星后不再命中 | 4 星永久常驻(降星逻辑死代码) | 15 秒后降回 3 星, 蓄力条缓慢漏气 |
| 劈棍/大圣 4 星(切式继承等) | 置数点不可达, 永不降星 | 与其它两式一致: 命中开/刷新 15 秒窗口, 到期降 3 星 |
| 降星时长 | 无统一来源(劈棍/大圣各持一份 300 常量, 立棍/戳棍不生效) | Config `charged4_window_ticks` 一处配置四式生效 |

## 5. 验证

`gradlew compileJava` 通过, 69 个警告均为项目原有 Epic Fight API 弃用警告。进游戏验证清单:

1. 四种棍式下铜头铁臂格挡成功 → 0→2 / 1→3 / 2→4 / 3→4 跳星(3 星亦可上 4), 满星不变;
2. 任一式 4 星命中敌人 → 开始 15 秒计时; 持续命中不降星; 脱战 15 秒后降回 3 星;
3. 配置文件修改 `charged4_window_ticks` 后四式同步生效;
4. 戳棍 4 星(铜头铁臂跳星后)按重击直接放凤穿花的既有行为不受影响。

## 6. 遗留事项

1. 第六部分遗留第 1 条(Thrust_CHARGED4_TIMER 死代码)已随本次清理一并解决;
2. 劈棍/大圣如需不依赖铜头铁臂的常规 4 星获取路径(如破条加第 4 档阈值), 属玩法设计决策, 本次未加;
3. `THRUST_FASHU_TIMER` 只写不读(置 20 + 递减, 无任何读取点), 属死键, 本次未动; `PILLAR_FASHU_TIMER` 为立棍专属机制(18 tick 内接重击消耗 1 星直接起立棍), 保持原样。

---

# 第七部分补丁: 3 星铜头铁臂可上 4 星（2026-09-09 实测反馈）

## 1. 问题现象

玩家实测: 3 星时使用铜头铁臂成功格挡, 星数不变, 上不去 4 星。

## 2. 原因

四个跳星消费块沿用立棍/戳棍原有门槛 `if (container.getStack() < 3)`: 3 星时跳星分支被整体跳过, 只清 FASHU_STACK 标记。即"2 星铜头直接满 4、3 星不跳"是本工程早前实现的刻意设计。另经反汇编核对: 原版 mod 的 Pillar/ThrustHeavyAttack updateContainer 中并无 FASHU_STACK 消费块, 原版 ShenfaTongtoutiebiSkill 也不加星——整个跳星机制(含 `< 3` 门槛)均为本工程新增, 放宽门槛不违背原版行为。

## 3. 修改内容

四处门槛统一 `stack < 3` → `stack < 4`:

- PillarHeavyAttack.java:483
- ThrustHeavyAttack.java:352
- SmashHeavyAttack.java:482
- GreatSageHeavyAttack.java:384

其余 `< 3` 判断(蓄力加成 smash:533 / pillar:515 / thrust:410, 破条加星四处 breakProgress)未动。跳星音效取新星级 `XULI_LEVEL[新星级-1]`, 3→4 跳星播第 4 级蓄力音效。
...
`gradlew compileJava` 通过。进游戏验证: 3 星状态铜头铁臂格挡成功 → 四式一致直接满 4 星, 满 4 星命中敌人后四蓄窗口照常开启/刷新。

---

# 第八部分: 棍势衰减时机修复 —— 命中刷新四蓄窗口不再限于满 4 星（2026-09-11）

## 1. 问题现象

1. 3 星攒 4 星的过程中, 棍势条会持续减少, 与攻击获得的棍势互相抵消, 很难攒到 4 星;
2. 满 4 星 15 秒未命中降回 3 星后, 即使继续命中敌人, 棍势仍然每 tick 流失, 衰减永不停止。

## 2. 原因分析

四蓄窗口计时器(CHARGED4_TIMER)的"置数点"与"消耗点"条件不匹配:

1. 置数点(四套重击技能 SmashHeavyAttack/PillarHeavyAttack/ThrustHeavyAttack/GreatSageHeavyAttack 的 onInitiate 中 DEAL_DAMAGE_EVENT_DAMAGE 监听)写法为 if (container.isFull()), 即只有满 4 星时命中敌人才刷新计时器(isFull() 语义为 stack >= 4, 见第五部分);
2. 消耗点(四个类 updateContainer 服务端的"四蓄的掉棍势时间判断"块)条件为 timer == 0 且 stack >= 3, 即 3 星起就参与衰减。

两者叠加后: 3 星状态下计时器恒为 0(从未被刷新过), 衰减常驻 —— 此为现象一; 满 4 星窗口到期降回 3 星后不再是满星, 命中再也刷不新计时器, 衰减永久化 —— 此为现象二。该"满 4 星才刷新"的写法最初存在于劈棍/大圣, 第七部分统一四式时被沿用到立棍/戳棍(原版 mod 中立棍/戳棍置数点本就缺失, 见第七部分第 1 节)。

## 3. 修改内容明细

四个文件各一处同款改动(SmashHeavyAttack.java / PillarHeavyAttack.java / ThrustHeavyAttack.java / GreatSageHeavyAttack.java):

| 改动 | 说明 |
|---|---|
| 删除 DEAL_DAMAGE_EVENT_DAMAGE 置数点外的 if (container.isFull()) 判断 | 命中敌人即刷新 CHARGED4_TIMER = charged4_window_ticks(默认 300 = 15 秒), 不再限满 4 星; 注释同步更新为"命中敌人即开启/刷新四蓄窗口, 窗口内不掉棍势, 窗口结束满4星降回3星并开始衰减" |

updateContainer 的衰减判断未改动: timer == 0 且 stack >= 3 时每 tick 流失 CHARGING_SPEED/5(0.24); 计时器自然走完且仍满 4 星时先降回 3 星再开始衰减(满星降 3 星设计保留, 即方案 A, 避免"4 星但棍势条将空"的星级与条不一致)。

## 4. 修改前后对比

| 场景 | 修改前 | 修改后 |
|---|---|---|
| 3 星攒 4 星, 持续命中 | 棍势持续流失, 与获取抵消 | 不流失, 命中刷新 15 秒窗口 |
| 满 4 星, 持续命中 | 不流失(原本正常) | 不流失(不变) |
| 3 星以上, 15 秒未命中 | 满 4 星降 3 星后流失; 3 星直接流失 | 不变 |
| 流失中再次命中敌人 | 不停止(非满星不刷新计时) | 计时刷新, 流失停止 |
| 0~2 星命中 | 不刷新计时器 | 刷新计时器(该区间不参与衰减判断, 无副作用, 保证破条升 3 星时窗口连续) |

## 5. 验证

四个文件已按项目约定用 google-java-format(AOSP 风格)格式化; ./gradlew build 通过(仅项目原有的 Epic Fight API 弃用警告), 产物 build/libs/wukong-compatible-1.0.0-mc1.20.1-forge.jar。建议游戏内实测场景: 3 星持续攻击攒 4 星不再掉条; 满 4 星脱战 15 秒降 3 星并开始掉条; 掉条中命中敌人掉条停止。

## 6. 遗留事项

1. 蹭伤害口径: 凡玩家造成的伤害(含棍花格挡反击、铜头铁臂反弹等)都会刷新窗口, 与原满星逻辑的口径一致, 属预期行为;
2. 1~2 星棍势条本就不衰减(衰减条件为 stack >= 3), 本次未涉及;
3. 满 4 星窗口到期"直接降回 3 星"的设计按方案 A 保留, 未改为从满星直接缓慢衰减; 若日后要改, 需同时处理"星数不随条衰减"的一致性问题。

---

# 第九部分: 戳棍寸退闪避修复 + 进尺闪避联动 + 动画比较恒 false 修复（2026-09-13）

## 1. 现象

1. 戳棍退寸(寸退)期间应视作闪避免伤(赌胜), 实测总在寸退被打, 免伤从未触发;
2. (排查中发现的隐患)退寸动画若被进尺/搅棍提前打断, 免伤标记 `Thrust_STEOP_BACK` 残留为 true, 直到下一次普攻才清除, 期间全程免伤;
3. (排查中发现的隐性 bug)搅棍命中 +10 棍势、进尺/戳棍 3 蓄/立棍风云转命中击退(原含点燃)等命中分支从未生效;
4. (需求新增)退寸赌胜成功后, 接进尺期间也应有闪避判定;
5. (排查确认)金箍棒的发光/变色渲染判定从未生效: 蓄力释放/切手动画期间武器不变色, 只有数据键驱动的蓄力中变色有效;
6. (需求新增)进尺之后的衍生窗口内按重击不允许再次进尺。

## 2. 原因分析

**现象 1 —— 免伤窗口开启过晚**。免伤标记 `Thrust_STEOP_BACK` 由退寸动画(`WukongAnimations.THRUST_RETREAT`)的 TICK_EVENTS 在动画时间 0.366s 才置 true、1.166s 置 false(`BattleUnit.CUNTUI_JIESUO`/`CUNTUI_SHANGSUO`); 而退寸的后撤位移发生在动画前 0.1s(`thrust_retreat.json` 中 Root 关键帧 t=0.1 已后撤约 1.18 格)。动画播放速度为 1.5 倍(PLAY_SPEED_MODIFIER), 换算真实时间窗口为退寸开始后 0.24s~0.78s —— 玩家开始后撤的前 0.24 秒(约 5 tick)内没有任何闪避判定, 而实战"敌人出招→按退寸"的命中几乎全落在这段, 故表现为总在寸退被打; 等窗口开启时人物早已撤完站定脱离攻击范围, 窗口几乎用不上。对照劈棍识破: `IS_IN_SPECIAL_ATTACK` 在 SMASH_DERIVE1 的 ON_BEGIN 即置 true, 整段动画有判定, 因此识破正常。

**现象 2 —— 窗口关闭依赖 tick 事件**。1.166s 的关窗事件在动画被提前打断时不会触发, 标记残留。

**现象 3 —— 动画比较方向错误**。`ThrustHeavyAttack`/`PillarHeavyAttack` 的 DEAL_DAMAGE_EVENT_ATTACK 监听器用 `getPlayerFor(null).getAnimation().equals(WukongAnimations.XXX.get())` 比较当前动画, 即动画访问器(运行时类型 `AnimationManager.AnimationAccessorImpl`)对动画实例(StaticAnimation)做 equals; Epic Fight 20.14.17 的 `AnimationAccessorImpl.equals` 仅支持与访问器/ResourceLocation/String 比较, 对 StaticAnimation 实例恒返回 false。正确写法是访问器对访问器(参照 `StaffPassive` 中 `WukongAnimations.STAFF_AUTO4.equals(...)` 与 `ShenWaiShenFaSkill` 中 `animation.equals(WukongAnimations.STAFF_AUTO5)`)。

**现象 5 —— 同类比较错误的另一形态**。`JinGuBangRenderer` 用 `getPlayerFor(null).getAnimation() instanceof StaticAnimation` 判定当前动画再交给 `AnimationJudge` 按实例引用比较; 但动画播放器(`AnimationPlayer.play` 字段, 类型 `AssetAccessor`)保存的是动画访问器而非动画实例, 该 instanceof 恒为 false, 导致武器发光(贴图轮换/自发光)与切手/二/三/四星变色全部不生效, 只剩 `isCharging`(数据键驱动)的蓄力中变色有效。修复方式: `AnimationJudge` 各判定方法改为接收 `AssetAccessor<? extends DynamicAnimation>`, 列表匹配由 `accessor.get() == 实例` 改为 `List.contains`(访问器 equals 按注册名比较), 调用方去掉 instanceof。

**现象 6 —— 窗口来源不可区分**。进尺接触帧后开启的 30 tick 搅棍窗口复用 `CAN_SECOND_TIMER`, 与退寸后窗口同键, 导致窗口内按重击会再次触发进尺(0 星弱化版可免费连发)。修复方式: 新增 `Thrust_FOOTAGE_WINDOW` 标记, 进尺释放时置 true、退寸开始时重置 false, `executeOnServer` 的 `CAN_SECOND_TIMER` 分支追加 `!Thrust_FOOTAGE_WINDOW` 条件, 进尺后按重击落入蓄力分支。

## 3. 修改内容明细

| 文件 | 改动 | 说明 |
|---|---|---|
| `WukongSkillDataKeys.java` | 新增 `Thrust_RETREAT_SUCCESS` 布尔数据键(默认 false) | 退寸赌胜成功标记, 作为进尺期间免伤的依据 |
| `WukongAnimations.java` | 退寸动画 ON_BEGIN 追加置 `Thrust_STEOP_BACK=true`、`Thrust_RETREAT_SUCCESS=false` | 免伤窗口从退寸第一帧开启, 同时重置上一轮的赌胜成功标记 |
| `WukongAnimations.java` | 退寸动画新增 ON_END 事件调 `BattleUnit.CUNTUI_SHANGSUO` | 动画结束或被打断时关闭免伤窗口, 消除标记残留 |
| `WukongAnimations.java` | 进尺动画 ON_END 追加置 `Thrust_RETREAT_SUCCESS=false` | 进尺结束或被打断时消耗赌胜成功标记 |
| `ThrustHeavyAttack.java` | TAKE_DAMAGE_EVENT_ATTACK 退寸免伤分支追加置 `Thrust_RETREAT_SUCCESS=true` | 赌胜成功时记录标记 |
| `ThrustHeavyAttack.java` | TAKE_DAMAGE_EVENT_ATTACK 新增分支: 标记为 true 且当前动画为进尺(访问器对访问器比较)时, 播残影+完美闪避音效并 `setCanceled(true)` 免伤 | 赌胜成功后的进尺全程闪避判定, 不再重复加棍势 |
| `ThrustHeavyAttack.java` | DEAL_DAMAGE_EVENT_ATTACK 4 处 `.equals(WukongAnimations.XXX.get())` 去掉 `.get()` 改为访问器对访问器 | 修复搅棍加棍势/进尺击退/3 蓄击退/风云转击退恒不生效 |
| `PillarHeavyAttack.java` | DEAL_DAMAGE_EVENT_ATTACK 1 处同上修复 | 修复风云转命中 +5 棍势恒不生效 |
| `WukongSkillDataKeys.java` | 新增 `Thrust_FOOTAGE_WINDOW` 布尔数据键(默认 false) | 进尺后窗口标记, 窗口内禁止再次进尺 |
| `ThrustHeavyAttack.java` | `executeOnServer` 的 `CAN_SECOND_TIMER` 分支追加 `!Thrust_FOOTAGE_WINDOW` 条件 | 进尺后按重击不再触发进尺, 落入蓄力分支 |
| `ThrustHeavyAttack.java` | `updateContainer` 进尺释放分支置 `Thrust_FOOTAGE_WINDOW=true` | 标记后续窗口来自进尺 |
| `WukongAnimations.java` | 退寸动画 ON_BEGIN 追加置 `Thrust_FOOTAGE_WINDOW=false` | 新一轮退寸重新允许进尺 |
| `AnimationJudge.java` | isGh/isGlow/isQie/isTwo/isThree/isFour 入参由 `StaticAnimation` 改为 `AssetAccessor<? extends DynamicAnimation>`, `contains` 改为 `List.contains` 按注册名比较 | 修复渲染端动画判定恒 false |
| `JinGuBangRenderer.java` | 6 处 `instanceof StaticAnimation` 模式移除, 直接传当前动画访问器给 `AnimationJudge` | 武器发光/变色判定恢复生效 |

## 4. 修改前后对比

| 场景 | 修改前 | 修改后 |
|---|---|---|
| 敌人攻击落在退寸开始 0.24s 内 | 被打(窗口未开) | 赌胜免伤 +30 棍势 + 残影音效 |
| 敌人攻击落在退寸 0.24s~0.78s | 免伤(但基本打不到) | 免伤(不变) |
| 退寸被进尺/搅棍打断 | 免伤标记残留, 之后全程免伤直到下次普攻 | ON_END 立即关窗, 无残留 |
| 赌胜成功后接进尺被打 | 被打 | 免伤(视作闪避, 不再加棍势) |
| 搅棍命中 | 不加棍势(分支恒 false) | +5 棍势(按需求由 +10 下调) |
| 进尺/3 蓄/风云转命中 | 无击退(分支恒 false) | 按文档描述击退(点燃已按需求移除) |
| 蓄力释放/切手动画期间武器外观 | 发光/变色判定恒 false, 不生效 | 按星数/动画正常发光变色 |
| 进尺后窗口内按重击 | 再次进尺(0 星可免费连发) | 不再进尺, 进入蓄力 |

## 5. 验证

`./gradlew compileJava` 通过(仅项目原有的 Epic Fight API 弃用警告); 改动文件已按项目约定用 google-java-format(AOSP 风格)格式化。建议游戏内实测: 敌人出招瞬间退寸可触发赌胜免伤; 退寸后秒接进尺/搅棍不再残留免伤; 赌胜成功后进尺期间被打不掉血; 搅棍命中棍势 +5; 进尺命中目标被击退且不点燃; 蓄力释放/切手技期间金箍棒按星数发光变色; 进尺后长按普攻接搅棍, 按重击进入蓄力而非再次进尺。

## 6. 遗留事项

1. 寸退免伤窗口现为退寸第一帧~动画时间 1.166s(约实际 0.78s)或动画被打断, 若手感偏长可调退寸动画 1.166F 的关窗 tick 事件;
2. 赌胜成功后若放凤穿花(满 4 星)而非进尺, 成功标记不会被消耗但也不产生效果(仅进尺动画读取该标记), 下次退寸开始时自动重置;
3. 本次同步的需求性调整(击退去点燃/0 星弱化进尺 x2.0/进尺后长按普攻接搅棍/进尺后禁止再进尺)记录于 docs/技能招式文档.md 第 10 节; 进尺后窗口内按重击转为蓄力前摇, 若希望此时按重击完全无反应, 可改 `executeOnServer` 分支为直接返回;
4. 金箍棒渲染端动画判定已改为访问器比较, 但 `AnimationJudge` 的动画清单(GLOW/ONE_STOR~FOUR_STOR)只覆盖劈棍/戳棍/立棍的星级蓄力动画, 大圣式重击动画不在清单内, 大圣式期间的变色由棍式专属分支(红橙半透明)负责, 属既有设计;
5. `ThrustHeavyAttack` 的击退分支中保留了引用立棍动画(风云转)的跨棍式判定, 该分支在戳棍技能下不会触发(立棍动画由立棍技能播放), 属原代码遗留, 未做清理。

---

# 第十部分: 斩棍式 0 星崩溃修复 + 切手技无棍势弱化版统一 + 江海翻命中解锁与耗全部棍势增伤 + 搅棍重击后入口（2026-09-13）

## 1. 现象

1. 斩棍式(劈棍二段衍生)在 0 星时按重击直接崩溃: `WuKongSounds.stackSounds.get(-1)` 数组越界, 且 `setStackSynchronize(-1)` 会把星数设成负数, 斩棍式在 0 星时根本放不出来;
2. 各切手技 0 星行为不一致: 破棍式/进尺已可放弱化版, 风云转/江海翻 0 星时按键无反应(`&& stackConsumed` 拦截);
3. 江海翻窗口由风云转收尾动画(`PILLAR_HEAVY_FENGYUNZHUANEND` 的 InPeriodEvent)无条件开启, 风云转空转(未命中)也能接江海翻, 与"风云转打中"的前置设计不符; 且江海翻固定只消耗 1 星, 无爆发定位;
4. 搅棍只能接在退寸/进尺之后, 戳棍重击后无法接搅棍。

## 2. 原因分析

**现象 1** —— 斩棍式分支(`SmashHeavyAttack.executeOnServer` 的 `CAN_SECOND_DERIVE` 分支)无条件播放星级音效并 `stack - 1`, 破棍式 0 星弱化修复(第九部分前的历史提交)只覆盖了一段衍生, 二段衍生漏改。

**现象 2** —— 立棍两个衍生分支都带 `&& stackConsumed` 条件, 0 星时直接落到蓄力分支。

**现象 3** —— `PILLAR_JIANGHAIFAN_TIMER` 此前只在风云转收尾动画播放期间被持续置数, 与是否命中无关。

**现象 4** —— 搅棍窗口(`REPEATING_DERIVE_TIMER`/`CAN_SECOND_TIMER`)只有退寸动画 0.366s 处与进尺动画 2.5s 处两个置位点。

## 3. 修改内容明细

| 文件 | 改动 | 说明 |
|---|---|---|
| `SmashHeavyAttack.java` | 斩棍式分支: 仅 `stack > 0` 时才播音效并消耗 1 星, 0 星直接播放斩棍动画 | 修复 0 星崩溃, 0 星可放弱化版 |
| `SmashHeavyAttack.java` | 斩棍式伤害 switch 的 default(0 星)由 4.48 改为 2.4 | 约 1 星 4.7 的一半, 与破棍式 1.0/1.96 同口径; 1/2/3 星 4.7/4.9/5.1 随星递增不变 |
| `PillarHeavyAttack.java` | 风云转分支去掉 `&& stackConsumed`, 0 星不播音效不耗星 | 0 星可放弱化版 |
| `PillarHeavyAttack.java` | 江海翻分支去掉 `&& stackConsumed`; 有星时改为 `resetConsumption` 消耗所有棍势 | 江海翻倾尽棍势的定位 |
| `PillarHeavyAttack.java` | DEAL_DAMAGE_EVENT_ATTACK 监听器: 风云转命中追加刷新 `PILLAR_JIANGHAIFAN_TIMER` 30 tick + 0 星时伤害 x0.5; 新增江海翻分支按 `STARS_CONSUMED` 加倍率(1 星 x1.0/2 星 x2.0/3 星 x3.0/4 星 x4.0/0 星 x0.5, 后续按需求由 1.15/1.3/1.5 上调为 1~4 倍) | 江海翻命中解锁 + 耗全部棍势增伤 + 弱化版 |
| `PillarHeavyAttack.java` | 删除不再使用的局部变量 `stackConsumed`, 新增 `ValueModifier` 导入 | 清理 |
| `WukongAnimations.java` | 风云转收尾动画的 InPeriodEvent 移除 `PILLAR_JIANGHAIFAN_TIMER` 置位(保留防坠机清除), 删除不再使用的 `Config` 导入 | 江海翻改为命中解锁 |
| `ThrustHeavyAttack.java` | `updateContainer` 服务端新增: 戳棍重击(animations 数组, 含凤穿花)播放到动画时间 0.9s(各重击接触帧 0.733~0.833s 之后)后, 每 tick 刷新 `REPEATING_DERIVE_TIMER`/`CAN_SECOND_TIMER` 30 tick 并置 `Thrust_FOOTAGE_WINDOW` | 戳棍重击后按住攻击键可接搅棍, 且该窗口禁止再进尺 |
| `WukongSkillDataKeys.java` | `Thrust_FOOTAGE_WINDOW` 注释更新 | 语义扩展为"进尺后或戳棍重击后窗口禁止再进尺" |

## 4. 修改前后对比

| 场景 | 修改前 | 修改后 |
|---|---|---|
| 0 星放斩棍式 | 崩溃(数组越界)/星数变负 | 放出弱化版 x2.4, 不耗星 |
| 0 星放风云转 | 无反应 | 放出弱化版每段 x0.56, 不耗星 |
| 0 星放江海翻 | 无反应 | 放出弱化版 x0.5, 不耗星 |
| 风云转空转后接江海翻 | 可以(收尾动画无条件开窗) | 不可以, 必须风云转打中 |
| 风云转旋转中接江海翻 | 必须松手等收尾动画 | 命中后直接按重击即可接 |
| 江海翻消耗与伤害 | 固定耗 1 星, 固定 0.9+4.48 | 耗所有棍势, 按星数 x1.0~x4.0 增伤 |
| 戳棍重击后按住攻击键 | 无反应/开始普攻连段 | 接搅棍(接触帧后 30 tick 窗口) |

## 5. 验证

`./gradlew compileJava` 通过; 改动文件已按项目约定用 google-java-format(AOSP 风格)格式化; `Config`/`stackConsumed` 等失效引用已清理。建议游戏内实测: 0 星放斩棍/风云转/江海翻不再崩溃且伤害明显低于正常版; 风云转命中后 30 tick 内可接江海翻, 空转不可; 江海翻放完棍势清空且高星时伤害明显提升; 戳棍 0~4 星重击(含凤穿花)后按住攻击键接搅棍, 按重击则进入蓄力。

## 6. 遗留事项

1. 各弱化版倍率(破棍 x1.0/斩棍 x2.4/风云转 x0.56 每段/江海翻 x0.5/进尺 x2.0)按"约为正常版一半"统一口径设定, 如需调整平衡改各技能的伤害 switch/分支即可;
2. 江海翻窗口为风云转最后一次命中后 30 tick, 收尾动画(约 17 tick)内及结束后约 13 tick 内均可按重击接续, 若手感偏紧可调大刷新值;
3. 江海翻的星数倍率写在 `PillarHeavyAttack` 的 DEAL_DAMAGE_EVENT_ATTACK 监听器中, 动画自带倍率(0.9/4.48)在 `WukongAnimations.PILLAR_HEAVY_RIVERSEAFLIP` 定义中, 两处共同决定最终伤害;
4. 搅棍重击后入口在接触帧(动画时间 0.9s)后每 tick 刷新窗口, 重击动画播放期间按住攻击键会在接触帧后立即接搅棍(打断重击收尾), 与进尺接搅棍行为一致;
5. 大圣式两段衍生已在后续统一: 一段衍生本就免费不耗星无需改动; 二段衍生最终定为"一段打中解锁 + 有星耗 1 星/0 星免费弱化 + 伤害固定两档(斩棍 x4.7/x2.4, 江海翻自带/减半, 不随棍势加伤害)"(见 docs/技能招式文档.md 第 7.2 节与第 10 节变动记录); 大圣式的音效调用本就有 `stack > 0` 保护, 无 0 星崩溃风险。

---

# 第十部分补丁: 进尺后不按任何键连续自动进尺（2026-09-13 实测反馈）

## 现象

退寸后按重击放出进尺, 之后不按任何键, 进尺会一直连续自动释放。

## 原因

进尺请求标记 `THRUST_METERS_BACK` 只置位不消耗: 进尺分支(`ThrustHeavyAttack.updateContainer`)播进尺动画时只清了 `CAN_SECOND_TIMER`, 没有把 `THRUST_METERS_BACK` 置回 false(此前仅在普攻动作事件里复位)。本轮新增的"进尺接触帧后重开 30 tick 搅棍窗口"把 `CAN_SECOND_TIMER` 重新置 30 后, 下一 tick 判定链 `CAN_SECOND_TIMER > 0 + 未按攻击键 + THRUST_METERS_BACK 仍为 true` 再次命中进尺分支, 形成"进尺→重开窗口→再进尺"的无限循环。

## 修改

`ThrustHeavyAttack.updateContainer` 进尺分支消耗请求时追加 `THRUST_METERS_BACK = false`(1 行)。复位后: 进尺接触帧后重开的窗口内不按攻击键不会再触发进尺; 按住攻击键正常接搅棍; 按重击进入蓄力(受 `Thrust_FOOTAGE_WINDOW` 禁止再进尺约束)。搅棍分支优先级高于进尺分支, 请求标记若在搅棍中残留, 会由下一次普攻动作事件复位(退寸必经普攻解锁, 无残留路径)。

## 验证

`./gradlew build` 通过。建议实测: 退寸→进尺后不按任何键, 进尺只放一次; 进尺后长按普攻接搅棍、按重击进蓄力均正常。
