...
public class WukongMoveCoordFunctions extends MoveCoordFunctions {
    ...
    public static final MoveCoordSetter TRACE_TARGET_DASH = (self, entitypatch, destSheet) -> {
        LivingEntity attackTarget = entitypatch.getTarget();
        if (attackTarget == null && entitypatch.getOriginal() instanceof Player player) {
            LivingEntity closestMonster = null;
            double closestDistance = Double.MAX_VALUE;
            List<LivingEntity> nearbyEntities = player.level().getEntitiesOfClass(LivingEntity.class, player.getBoundingBox().inflate(10.0D), entity -> entity instanceof Monster && entity.isAlive() && !isFakeWukong(entity));
        ...
        Vec3 startPos = entitypatch.getAnimator().getVariables().getOrDefault(ActionAnimation.BEGINNING_LOCATION, self.getRealAnimation());
        if (startPos == null) {
        ...
        Vec3 dashDirection = horizontalDistance > 0.01D ? horizontalToTarget.normalize() : Vec3.ZERO;
        float stopDistance = (attackTarget.getBbWidth() + ((LivingEntity) entitypatch.getOriginal()).getBbWidth()) * 0.75F;
        double dashDistance = Math.min(Math.max(horizontalDistance - stopDistance, 0.0D), MAX_DASH_DISTANCE);
