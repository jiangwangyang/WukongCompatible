...
public class WukongKeyMappings {
    ...
    public static final MyKeyMapping GREATSAGE_STYLE = new MyKeyMapping("key.wukong.greatsage_stance", GLFW.GLFW_KEY_G, "key.wukong.category");
        ...
        public static void onClientTick(TickEvent.ClientTickEvent event) {
            ...
            localPlayer.getCapability(EpicFightCapabilities.CAPABILITY_SKILL).ifPresent(capabilitySkill -> {
                for (Skill selectableSkill : WukongSkills.getSelectableSkills()) {
                    if (selectableSkill != null && !capabilitySkill.hasLearned(selectableSkill)) {
                        capabilitySkill.addLearnedSkill(selectableSkill);
                    }
                }
                Skill skill = null;
                SkillContainer weaponInnate = patch.getSkill(SkillSlots.WEAPON_INNATE);
                if (weaponInnate != null && !weaponInnate.isEmpty()) {
                    ...
                    boolean isCommonCharging = (weaponSkill == WukongSkills.SMASH_HEAVY_ATTACK
                            || weaponSkill == WukongSkills.PILLAR_HEAVY_ATTACK
                            || weaponSkill == WukongSkills.GREATSAGE_HEAVY_ATTACK)
                            && dataManager.hasData(WukongSkillDataKeys.IS_CHARGING.get())
                            && dataManager.getDataValue(WukongSkillDataKeys.IS_CHARGING.get());
                    boolean isThrustCharging = weaponSkill == WukongSkills.THRUST_HEAVY_ATTACK
                ...
                if (skillContainer != null && skill != null && skillContainer.getSkill() != skill) {
                    ...
                    PacketRelay.sendToServer(PacketHandler.INSTANCE, new UpdateWeaponInnatePacket(styleIndex));
                }
            });
